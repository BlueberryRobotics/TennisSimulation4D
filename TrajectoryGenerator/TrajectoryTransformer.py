...
- Trajectory4DLibraryXXX.pkl/pk1 -> points table points
- GenXXReference.parquet -> reference table references
- Default output DB name: TrajectoryXXXReferenceXX.db
...
def extract_reference_version_from_parquet(parquet_path: Optional[Path]) -> str:
    ...
    match = re.search(r"Gen(\d+)Reference", stem, re.IGNORECASE)
    if match:
        return match.group(1)
    trailing_digits = re.search(r"(\d+)$", stem)
    if trailing_digits:
...
def main():
    ...
    if args.sqlite is None:
        args.sqlite = args.pkl.parent / f"Trajectory{trajectory_version}Reference{reference_version}.db"
...
# Example command:
# python TrajectoryGenerator/TrajectoryTransformer.py --pkl Trajectory4DLibrary005.pkl --gen-reference-parquet GamePlay/Gen11Reference.parquet
# you can override the db name by specifying --sqlite