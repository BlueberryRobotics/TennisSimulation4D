...
# version 009 collapses defensiveCol/defensiveRow during consolidation
# while keeping simulation shot outputs unchanged.
# It also preserves downhillSpeed in grouping keys so downhill variants
# remain distinct through consolidation.
...
def main(
    ...
    inputHasDownhillSpeed = _HasParquetColumn(conn, input_pattern, "downhillSpeed")
    inputDownhillSpeedExpr = (
        "COALESCE(TRY_CAST(downhillSpeed AS DOUBLE), 0.0) AS downhillSpeed"
        if inputHasDownhillSpeed
        else "0.0 AS downhillSpeed"
    )

    previousWinnerExpr = "FALSE AS winner"
    previousDownhillSpeedExpr = "0.0 AS downhillSpeed"
    if previous_file:
        ...
        previousHasDownhillSpeed = _HasParquetColumn(conn, previous_file, "downhillSpeed")
        previousDownhillSpeedExpr = (
            "COALESCE(TRY_CAST(downhillSpeed AS DOUBLE), 0.0) AS downhillSpeed"
            if previousHasDownhillSpeed
            else "0.0 AS downhillSpeed"
        )
    ...
    # Defensive columns are intentionally not part of the key in this version.
    # ----------------------------
    conn.execute(f"""
    ...
    if previous_file:
        ...
        conn.execute("""
            ...
            LEFT JOIN new_mirrored_agg n
                ...
                AND p.bc = n.bc
                AND p.br = n.br
                AND p.downhillSpeed = n.downhillSpeed
...
if __name__ == "__main__":
    ...
    parser.add_argument("--minPrevCount", type=int, default=1)
...
# Usage example:
# python ConsolidateResults008.py shots_*.parquet ConsolidatedGen0_008.parquet
# python ConsolidateResults008.py shots_*.parquet ConsolidatedGen1_008.parquet --previousFile ConsolidatedGen0_008.parquet --minPrevCount 1 --minWinPct 0.2

# Sequential first run has no previous consolidation file
# python IterativeSimulation/RunConsolidation008.py `
#   --mode merge `
#   --inputFolder IterativeSimulation `
#   --filePattern "shots_*.parquet" `
#   --outputFile "IterativeSimulation/ConsolidatedGen2_008.parquet" `

# Merge mode with previous file
# python IterativeSimulation/RunConsolidation008.py `
#   --mode merge `
#   --inputFolder IterativeSimulation `
#   --filePattern "shots_*.parquet" `
#   --outputFile "IterativeSimulation/ConsolidatedGenAll_008.parquet" `
#   --previousFile "IterativeSimulation/ConsolidatedGen23_008.parquet"

# Sequential by date consolidations starting with existing previous consolidated file
# and creating new consolidated generation with each subsequent shots file
# python IterativeSimulation/RunConsolidation008.py `
#   --mode sequential `
#   --inputFolder IterativeSimulation `
#   --filePattern "shots_*.parquet" `
#   --outputTemplate "IterativeSimulation/ConsolidatedGen{gen}_008.parquet" `
#   --startGen 24 `
#   --previousFile "IterativeSimulation/ConsolidatedGen23_008.parquet"
